  ...
  rotate(degrees, mode, cb) {
    ...
    if (typeof mode !== "boolean" && typeof mode !== "string") {
      return throwError.call(
        this,
        "Mode must be of type Boolean or String",
        cb
      );
    }

    try {
      let { width } = this.bitmap;
      let { height } = this.bitmap;
      ...
      if (mode) {
        ...
        width =
          Math.ceil(
            Math.abs(originalWidth * cosine) + Math.abs(originalHeight * sine)
          ) + 1;
        height =
          Math.ceil(
            Math.abs(originalWidth * sine) + Math.abs(originalHeight * cosine)
          ) + 1;
        ...
        const newWidthHeight = ensureInteger(
          Math.max(width, height, originalWidth, originalHeight)
        );
