...
const bitmapDenyProxy = new Proxy(
  {},
  {
    get(target, prop) {
      throw new Error(
        `Attempted to access property "${prop}" within bitmap data wile C++ thread is working on it.`
      );
    },
  }
);
...
exports.cppCallbackWrapper = (jimpInstance, userCallback) => {
  if (
    !jimpInstance.constructor.__trueAsync ||
...
exports.isSynchronous = (jimpInstance, cb) => {
  return (
    !jimpInstance.constructor.__trueAsync ||
    (jimpInstance.constructor.__trueAsync && typeof cb !== "function")
  );
};
