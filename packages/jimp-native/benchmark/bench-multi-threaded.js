...
function printReport() {
  ...
  for (const task of TASKS) {
    ...
    for (const stage of settings.stages) {
      ...
      const nativeResult = nativeResults.find(
      ...
      console.log(
        STAGE_INDENT,
        `STAGE: ${stage} ` +
          `JS (${settings.benchmarkRepeat} CALLS): ${formatTime(
            jsResult.nanoseconds
          )} ` +
          `NATIVE (${settings.benchmarkRepeat} CALLS): ${formatTime(
            nativeResult.nanoseconds
          )} ` +
          `SPEED IMPROVEMENT: ${(
            jsResult.nanoseconds / nativeResult.nanoseconds
          ).toFixed(2)}x`
      );
    }
  }
