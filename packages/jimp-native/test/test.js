...
async function runTestsWithConstructor(JimpConstructor, constructorName) {
  for (const test of tests) {
    const updateLog = console.draft(
      `(${chalk.gray(constructorName)}) ${chalk.yellow(
        test.name
      )} (${chalk.cyanBright(getFrame())})`
    );
    ...
    const logUpdateInterval = setInterval(() => {
      updateLog(
        `(${chalk.gray(constructorName)}) ${chalk.yellow(
          test.name
        )} ${chalk.gray(currentPhaseName)} (${chalk.cyanBright(getFrame())})`
      );
    }, SPINNER_INTERVAL);
      ...
      try {
        ...
        errorsEncountered.push(
          new TestError(
            ...
            phase.name,
            constructorName,
            phase.type !== testConstants.PHASE_TYPES.NO_THROW
          )
        );
        console.log(
          `(${chalk.gray(constructorName)}) ${chalk.yellow(
            test.name
          )} ${chalk.gray(phase.name)} (${chalk.redBright("TEST ERROR")})`
        );
        continue;
      }
    }
...
async function compareTestResults() {
  const updateLog = console.draft(
    `${chalk.yellow("Running binary comparisons...")}`
  );
  const updateInterval = setInterval(() => {
    updateLog(
      `${chalk.yellow("Running binary comparisons...")} (${chalk.cyanBright(
        getFrame()
      )})`
    );
  }, SPINNER_INTERVAL);
  ...
  for (const test of tests) {
    ...
    phaseLoop: for (const phase of test.phases) {
      const phaseKey = `${test.name}-${phase.name}`;
      const resultPair = imageBufferFiles.filter(
        (fileName) =>
          fileName === `${phaseKey}-js.bin` ||
          fileName === `${phaseKey}-native.bin`
      );

      if (resultPair.length !== 2) {
        // Only complain if no test failure occurred.
        if (
          errorsEncountered.filter(
            (err) => err.testName === test.name && err.testPhase === phase.name
          ).length === 0
        ) {
          errorsEncountered.push(
            new ComparisonError(
              `One or both bin files are missing for test "${phaseKey}"`
            )
          );
        }
      ...
      for (let i = 0; buffer1.length > i; i++) {
        ...
        if (diff > (test.tolerance || testConstants.COLOUR_TOLERANCE)) {
          if (
            i < 4 &&
            (test.name === "composite" || phase.name === "composite")
          ) {
          ...
          errorsEncountered.push(
            new ComparisonError(
              `Pixel components from "${phaseKey}" differ by more than ${
                test.tolerance || testConstants.COLOUR_TOLERANCE
              }.\n` +
                `First error encountered at byte index: ${i}. Difference: ${diff}. Component: ${
                  COMPONENT_MAP[i % 4]
                }`
            )
          );
  ...
  updateLog(
    `${chalk.yellow("Running binary comparisons...")} (${chalk.green("Done")})`
  );
}
...
function printReport() {
  ...
  if (errorsEncountered.length === 0) {
    console.log(
      chalk.green(
        "All tests passed. Native functions match original within tolerances.\n"
      )
    );
