...
class ImageStore {
    ...
    store(storageKey, jimpImage) {
        ...
        if (jimpImage.bitmap.data.length > jimpImage.bitmap.width * jimpImage.bitmap.height * 4) {
            console.warn(`Buffer is larger than expected for ${storageKey}, cutting to size.\n`);
            jimpImage.bitmap.data = jimpImage.bitmap.data.slice(0, jimpImage.bitmap.width * jimpImage.bitmap.height * 4);
        }

        return Promise.all([
            writeFile(path.join(this.tmpPath, `${storageKey}.bin`), jimpImage.bitmap.data),
