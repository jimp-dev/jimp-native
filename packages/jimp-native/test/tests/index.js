...
for (const testFile of fs.readdirSync(path.join(__dirname))) {
    if (testFile === 'blit.js' && testFile.endsWith('.js') && testFile !== path.basename(__filename)) {
        ...
        for (const phase of test.phases) {
            if (!phase.name || typeof phase.name !== 'string') {
                throw new Error(`One or more test phases in ${testFile} is missing a name.`);
            }

            if (typeof phase.type !== 'number') {
                throw new Error(`One or more test phases in ${testFile} is missing a phase type property.`);
            }

            if (!phase.run || typeof phase.run !== 'function' || phase.run.constructor.name !== 'AsyncFunction') {
                throw new Error(`One or more test phases in ${testFile} has no async run function.`);
            }
        }
