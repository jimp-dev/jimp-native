...
for (let i = 0; process.argv.length > i; i++) {
    ...
    if (arg === '--stages') {
        if (process.argv.length - 1 === i) {
            console.error('Please specify desired stages (comma separated, no spaces) when using --stages.');
        ...
        for (const selectedStage of stagesSelected) {
            if (!BENCHMARK_STAGES.includes(selectedStage)) {
                console.error(`${selectedStage} is not a stage. Stages must be provided in a comma separated list (no spaces).`);
                printStagesAvailable();
                process.exit(1);
            }
        }
    ...
    if (arg === '--repeat') {
        ...
        try {
            benchmarkRepeat = Math.abs(parseInt(process.argv[i + 1]));
        } catch (err) {
